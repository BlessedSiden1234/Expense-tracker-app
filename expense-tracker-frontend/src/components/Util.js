...
// Add at the top or near the other helpers
export const normalizeExpenses = (rawExpenses) => {
    if (!rawExpenses) return [];
    return Object.keys(rawExpenses).map((key) => {
        const expense = rawExpenses[key];
        return {
            key, // preserve the unique key
            ...expense
        };
    });
};
